...
describe("Http layer", () => {
  ...
  it(`should delete a record when id is within the criteria`, (done) => {
    const request = new HttpRequest({criteria: {id: deadpoolResponse.data["id"]}});
    adapter.destroy(resource, request)
  ...
  it(`should delete a record when id is within the data`, (done) => {
    const request = new HttpRequest({data: deadpoolResponse.data});
    adapter.destroy(resource, request)
